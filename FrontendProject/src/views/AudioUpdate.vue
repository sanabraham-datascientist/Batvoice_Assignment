    ...
    <div v-for="(segment, id) in segments" :key="id">
      <div class="flex flex-row justify border-b-2 border-gray-200">
        <div class="ml-20 mb-3">
          <audio controls>
            <source :src=segment.file_name type="audio/mpeg" />
            <source :src=segment.file_name type="audio/wav" />
          </audio>
          <p class="text-base font-medium text-navy-700 dark:text-white">
            <!-- Transcripted by {{audio.user_name}} at{{ audio.updated_at }}  -->
          </p>
        </div>
        <div class="ml-10">
          <div class="mt-4 mb-6">
    ...
    getAudio() {
      ...
      axios
        ...
        .then((response) => {
          ...
          this.segments = response.data.audio_segments;
        })
        .catch((error) => {
